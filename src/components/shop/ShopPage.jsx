...
export default function ShopPage() {
  ...
  // const { data, isLoading, error_product } = useGet('products');
...
// useEffect(() => {
//   setLoading(isLoading);   // better: use the built-in loading state if your hook provides it
...
//     // The real array is nested: response.data.data (the pagination.data)
...
// }, [data, isLoading, error]);
  ...
  const filtered = useMemo(() => {
    ...
    // Only keep your other filters (price, sale, stock, sorting)
    // if (onSale) {
    //   out = out.filter(p => (p.discountPercentage || 0) > 0);
    // }
    // if (inStock) {
                    ...
                    <div key={p.id} className="card">
                      <div className="media">
                        ...
                        <div className="iconsBox">
                          ...
                          //  onClick={() => addToWishlistManager(productData, wishlistExecute)}
                          onClick={() => {
                          if (!userId) {
